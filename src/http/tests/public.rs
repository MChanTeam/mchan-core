...
#[sqlx::test(migrator = "MIGRATOR")]
async fn staff_host_home_and_thread_show_controls_for_allowlisted_identity(pool: sqlx::SqlitePool) {
    let thread_id = sqlx::query_scalar::<_, i64>(
        "SELECT id FROM threads WHERE title = 'Welcome to Engineering' LIMIT 1",
    )
    .fetch_one(&pool)
    ...
    let reply_id = sqlx::query_scalar::<_, i64>(
        "SELECT id FROM replies WHERE body = 'Glad to be here.' LIMIT 1",
    )
    .fetch_one(&pool)
    .await
    .expect("seeded reply");
    ...
    let staff_home = send(
        &app,
        with_header(
            with_header(get_request("/"), "host", "staff.mchan.fyi"),
            "cf-access-authenticated-user-email",
            "MoDeRaToR@ExAmPlE.CoM",
        ),
    )
    .await;
    assert_eq!(staff_home.status(), StatusCode::OK);
    let staff_home_body = response_text(staff_home).await;
    assert!(staff_home_body.contains(r#"href="/admin""#));
    assert!(staff_home_body.contains(r#"href="/mod/reports""#));

    let staff_thread = send(
        &app,
        with_header(
            with_header(
                get_request(&format!("/threads/{thread_id}")),
                "host",
                "staff.mchan.fyi",
            ),
            "cf-access-authenticated-user-email",
    ...
    .await;
    assert_eq!(staff_thread.status(), StatusCode::OK);
    let staff_thread_body = response_text(staff_thread).await;
    assert!(staff_thread_body.contains(r#"class="direct-hide-form""#));
    assert!(staff_thread_body.contains(&format!(r##"href="#hide-thread-{thread_id}""##)));
    assert!(staff_thread_body.contains(&format!(r#"action="/mod/threads/{thread_id}/hide""#)));
    assert!(staff_thread_body.contains(&format!(r##"href="#hide-reply-{reply_id}""##)));
    assert!(staff_thread_body.contains(&format!(r#"action="/mod/replies/{reply_id}/hide""#)));
}

#[sqlx::test(migrator = "MIGRATOR")]
async fn staff_host_without_identity_hides_staff_controls(pool: sqlx::SqlitePool) {
    let thread_id = sqlx::query_scalar::<_, i64>(
        "SELECT id FROM threads WHERE title = 'Welcome to Engineering' LIMIT 1",
    )
    .fetch_one(&pool)
    ...
    assert_eq!(thread.status(), StatusCode::OK);
    let thread_body = response_text(thread).await;
    assert!(!thread_body.contains(r#"class="direct-hide-form""#));
